...
class DataGenerator(Sequence):
    ...
    def data_generation(self, dataset_batch):
        ...
        y = np.zeros((batch_size,) + self.img_size_after_preprocessing + (self.number_classes + 1,), dtype="float32")
        ...
        for i, (img_path, mask_path) in enumerate(dataset_batch):
            ...
            if img.shape[:2] != self.img_size_after_preprocessing:
                logger.printe(f"Image {img_path} has wrong size. Expected {self.img_size} but got {img.shape[:2]}. Try to set resize_height and resize_width to {self.img_size}")
                return None, None
            x[i] = img
